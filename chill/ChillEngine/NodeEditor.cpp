...
void NodeEditor::launchIcesl() {
  ...
  if ( pid == 0 ) {
    Instance()->m_icesl_p_info.hProcess = getpid();
    Instance()->m_icesl_p_info.hThread  = pthread_self();

    std::vector<char*> args{};
    args.push_back(&icesl_params[0]);

    execvp(icesl_path.generic_string().c_str(), args.data());
  }
#endif
}
...
#elif UNIX
  if (Instance()->m_icesl_p_info.hProcess > 1) {
    kill(Instance()->m_icesl_p_info.hProcess, SIGKILL);
    std::cerr << "Icesl will close in a few seconds..." << std::endl;
  }
#endif
}
...
int NodeEditor::launch()
{
  ...
  if (nodeEditor->m_auto_icesl) {
...
#elif UNIX
    Instance()->m_chill_hwnd = wmInfo.info.x11.window;
#endif
    ...
    //nodeEditor->m_icesl_window = SDL_CreateWindowFrom(reinterpret_cast<void*>(&nodeEditor->m_icesl_hwnd));
  ...
  while (!done)
  {
    ...
    while (SDL_PollEvent(&event))
    {
      ...
      if (event.type == SDL_WINDOWEVENT) {
        // Handle close event
        if (event.window.event == SDL_WINDOWEVENT_CLOSE && event.window.windowID == SDL_GetWindowID(nodeEditor->m_chill_window)) {
          done = true;
        }

        Instance()->manageWindowEvent(&event);
      }
...
void NodeEditor::setIceslPath() {
  ...
  if (!fs::exists(m_iceslPath)) {
    ...
    const char * modalText = "             Icesl was not found on this computer.            \n"
                             "   *------*                                         *------*  \n"
                             "  /|     /|  In order for Chill to work properly,   |\\     |\\  \n"
                             "(o)-----* |  it needs access to IceSL.              |(o)-----*  \n"
                             " | *----|-*                                         *-|----* |  \n"
                             " |/     |/   Do you want to specify the location of  \\|     \\|  \n"
                             " *-----(o)   IceSL's executable on your computer ?    *-----(o) \n";
    ...
    const SDL_MessageBoxColorScheme colorScheme = {
      { /* .colors (.r, .g, .b) */
        {  38,  38,  38 }, /* [SDL_MESSAGEBOX_COLOR_BACKGROUND]        */
        { 255, 255, 255 }, /* [SDL_MESSAGEBOX_COLOR_TEXT]              */
        {  73, 193, 194 }, /* [SDL_MESSAGEBOX_COLOR_BUTTON_BORDER]     */
        { 128, 128, 128 }, /* [SDL_MESSAGEBOX_COLOR_BUTTON_BACKGROUND] */
        { 255, 136,  51 }  /* [SDL_MESSAGEBOX_COLOR_BUTTON_SELECTED]   */
      }
    };
...
void NodeEditor::manageWindowEvent(const SDL_Event* event) {
  SDL_WindowEvent window = event->window;
  if (window.windowID == SDL_GetWindowID(m_chill_window)) {
